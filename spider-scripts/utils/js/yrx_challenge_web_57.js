    ...
    function l(q, v) {
        if (!S[q]) {
            if (!I[q]) {
                ...
                if (k)
                    return k(q, !0x0);
                var T = new Error('Cannot find module \'' + q + '\'');
            ...
            ;(function(S, X) {
                ...
                var S = S || function(X, l) {
                    ...
                    var q = function() {
                        ...
                        '_process': function(W) {
                            var p, e = this['_data'], t = e['words'], J = e['sigBytes'], O = this['blockSize'], m = O * 0x4, A = J / m;
        ...
        return (function() {
            var S = I
                ...
                'compute': function(T, z) {
                    var h, f = this['cfg'], R = f['hasher']['create'](), r = k['create'](), B = r['words'], V = f['keySize'], F = f['iterations'];
                    while (B['length'] < V) {
                        h && R['update'](h);
        ...
        return function(S) {
            var X = I
                ...
                '_doProcessBlock': function(p, e) {
                    ...
                    var A = this['_hash']['words'], c = f['words'], g = R['words'], j = K['words'], N = T['words'], a = z['words'], Z = h['words'], E, y, Y, x, d, U, D, u, s, Q;
                    U = E = A[0x0],
                    D = y = A[0x1],
                    u = Y = A[0x2],
                    s = x = A[0x3],
...
function decryptData(result) {
    // 导出解密函数
    const moduleLoader = P(param1, param2, param3);
    // 直接通过模块ID获取导出内容
    const decryptModule = moduleLoader(0x2);
    const decryptFunction = decryptModule.decrypt;

    try {
        const key = result.slice(0, 8);
        const encryptedData = result.slice(8);
        const decryptedText = decryptFunction(key, encryptedData);
        const decryptJsonData = JSON.parse(decryptedText)
